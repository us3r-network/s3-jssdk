...
export type VoteButtonProps = ButtonHTMLAttributes<HTMLButtonElement> & {
  ...
  votedIconComponent?: React.ComponentType;
  unVotedIconComponent?: React.ComponentType;
};

export default function VoteButton({ linkId, ...props }: VoteButtonProps) {
  ...
  return (
    <Button variant="vote" onClick={onVote} {...props}>
      {(loading && <ButtonLoading />) || (
        <>
          {!findCurrUserVote || findCurrUserVote?.node.revoke ? (
            <VoteIcon />
          ) : (
            <VoteWhiteIcon />
          )}
